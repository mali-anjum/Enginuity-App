-- v0.3 SPARK: support multi-tag filter combinations in global search
  ...
  select * from (
      ...
      where length(trim(coalesce(search_query, ''))) > 0
        ...
        and (
          ...
          or exists (
            ...
            where et.entity_type = 'experiment'
              and et.entity_id = e.id
              and lower(tg.name) = any (
                select lower(trim(value))
                from unnest(filter_tags) value
              )
          )
        )
        and (date_from is null or (e.created_at at time zone 'utc')::date >= date_from)
        and (date_to is null or (e.created_at at time zone 'utc')::date <= date_to)
      order by rank desc
    ...
    (
      ...
      where length(trim(coalesce(search_query, ''))) > 0
        ...
        and (date_from is null or (n.created_at at time zone 'utc')::date >= date_from)
        and (date_to is null or (n.created_at at time zone 'utc')::date <= date_to)
      order by rank desc
      limit greatest(1, least(coalesce(result_limit, 80), 200))
    )
    union all
    (
      ...
      where length(trim(coalesce(search_query, ''))) > 0
        ...
        and (date_from is null or (h.created_at at time zone 'utc')::date >= date_from)
        and (date_to is null or (h.created_at at time zone 'utc')::date <= date_to)
      order by rank desc
      limit greatest(1, least(coalesce(result_limit, 80), 200))
    )
  ) hits;
